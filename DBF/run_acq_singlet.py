...
def main():
    ...
    print(f'running ACQ singlet for {n_qubits} qubits, J={J}, D={D}, dt={dt}, N={N}, T={T}')
    ...
    print("Computing ACQ for singlet initial state")
    H_acq, H_trot = ham.Heisenberg_OBC_DN(J, n_qubits, sparse=True)

    qc_singlet = state_prep_singlet().qs.compile()
    psi_0_singlet = sp.csc_matrix(
        np.real_if_close(get_statevector(qc_singlet, n_qubits).reshape(-1, 1))
    )
    ...
    with open(f"Heisenberg_N{n_qubits}_J{J}_ACQ_D{D}_singlet_results_simulation.pkl", "wb") as f:
    ...
    with open(f"Heisenberg_N{n_qubits}_J{J}_ACQ_D{D}_singlet_results_transpile.pkl", "wb") as f:
        pickle.dump(circuit_data_singlet, f)
